...
#if 0
static void printmatrixuint8_t(uint8_t *m) {
	int i;
	for(i = 0; i < DSIZE2; i++) {
		printf("%3d ",m[i]);
		if((i+1) % 8 == 0) printf("\n");
...
static void idct(int *block,int used_col) {
	...
	for (i = 0; i < DSIZE; i++, ptr++) {
		...
		ptr[DSIZE * 1] = (tmp1 + tmp6); // oint16_t + od16
		ptr[DSIZE * 6] = (tmp1 - tmp6); // oint16_t - od16
...
void mdecInit(void) {
	...
	mdec.rl = (uint16_t *)&psxM[0x100000];
}
		...
		case 0x4: // quantization table upload
			{
				...
				// printmatrixuint8_t(p);
				// printmatrixuint8_t(p + 64);
				iqtab_init(iq_y, p);
...
{
	uint8_t *base = (uint8_t *)&psxM[0x100000];
	uint32_t v;
