...
const uint32_t HSyncTotal[2] = { 263, 313 };
...
static uint_fast8_t rcntFreezeLoaded = false;
...
INLINE void _psxRcntWcount( uint32_t index, uint32_t value )
{
    if( value > 0xffff )
    {
...
void psxRcntInitFromFreeze(void)
{
	...
	for (int i = 0; i < CounterQuantity; ++i)
	{
		...
		uint32_t count = (psxRegs.cycle - rcnts[i].cycleStart) / rcnts[i].rate;
