...
static void memstats_add_read(uint32_t addr, enum MemstatWidth width);
static void memstats_add_write(uint32_t addr, enum MemstatWidth width);
#else
static inline void memstats_reset() {}
static inline void memstats_print() {}
static inline void memstats_add_read(uint32_t addr, enum MemstatWidth width) {}
static inline void memstats_add_write(uint32_t addr, enum MemstatWidth width) {}
...
int8_t *psxR;
int8_t *psxH;
uint_fast8_t psxM_allocated;
uint_fast8_t psxP_allocated;
uint_fast8_t psxR_allocated;
uint_fast8_t psxH_allocated;

uint8_t **psxMemWLUT;
uint8_t **psxMemRLUT;

static uint8_t *psxNULLread;
...
int psxMemInit()
{
	...
	if (psxMemRLUT == NULL) { psxMemRLUT = (uint8_t **)calloc(0x10000, sizeof(void *)); }
	if (psxMemWLUT == NULL) { psxMemWLUT = (uint8_t **)calloc(0x10000, sizeof(void *)); }
	if (psxNULLread == NULL) { psxNULLread = (uint8_t*)calloc(0x10000, 1); }

	// If a dynarec hasn't already mmap'd any of psxM,psxP,psxH,psxR, allocate
	//  them here. Always use uint_fast8_ts 'psxM_allocated' etc to check allocation
	...
	if (!psxM_allocated) { psxM = (int8_t*)malloc(0x200000);  psxM_allocated = psxM != NULL; }

	// Allocate 64K for PSX ROM expansion 0x1f00_0000 region
	if (!psxP_allocated) { psxP = (int8_t*)malloc(0x10000);   psxP_allocated = psxP != NULL; }

	// Allocate 64K for PSX scratcpad + HW I/O 0x1f80_0000 region
	if (!psxH_allocated) { psxH = (int8_t*)malloc(0x10000);   psxH_allocated = psxH != NULL; }

	// Allocate 512KB for PSX ROM 0xbfc0_0000 region
	if (!psxR_allocated) { psxR = (int8_t*)malloc(0x80000);   psxR_allocated = psxR != NULL; }
...
// MemR
	...
	for (i = 0; i < 0x80; i++) psxMemRLUT[i + 0x0000] = (uint8_t *)&psxM[(i & 0x1f) << 16];

	memcpy(psxMemRLUT + 0x8000, psxMemRLUT, 0x80 * sizeof(void *));
	memcpy(psxMemRLUT + 0xa000, psxMemRLUT, 0x80 * sizeof(void *));

	psxMemRLUT[0x1f00] = (uint8_t *)psxP;
	psxMemRLUT[0x1f80] = (uint8_t *)psxH;

	for (i = 0; i < 0x08; i++) psxMemRLUT[i + 0x1fc0] = (uint8_t *)&psxR[i << 16];

	memcpy(psxMemRLUT + 0x9fc0, psxMemRLUT + 0x1fc0, 0x08 * sizeof(void *));
	memcpy(psxMemRLUT + 0xbfc0, psxMemRLUT + 0x1fc0, 0x08 * sizeof(void *));

// MemW
	for (i = 0; i < 0x80; i++) psxMemWLUT[i + 0x0000] = (uint8_t *)&psxM[(i & 0x1f) << 16];
	memcpy(psxMemWLUT + 0x8000, psxMemWLUT, 0x80 * sizeof(void *));
	memcpy(psxMemWLUT + 0xa000, psxMemWLUT, 0x80 * sizeof(void *));

	psxMemWLUT[0x1f00] = (uint8_t *)psxP;
	psxMemWLUT[0x1f80] = (uint8_t *)psxH;
...
void psxMemReset()
{
	...
	memset(psxP, 0xff, 0x10000); // PIO Expansion memor
...
void psxMemWrite32_CacheCtrlPort(uint32_t value)
{
	...
	static uint32_t mem_bak[0x10000/4];
#endif //PSXREC
	...
	switch (value)
	{
		case 0x800: case 0x804:
			...
			psxMemRLUT[0x0000] = psxMemRLUT[0x0020] = psxMemRLUT[0x0040] = psxMemRLUT[0x0060] = (uint8_t *)mem_bak;
			psxMemRLUT[0x8000] = psxMemRLUT[0x8020] = psxMemRLUT[0x8040] = psxMemRLUT[0x8060] = (uint8_t *)mem_bak;
			psxMemRLUT[0xa000] = psxMemRLUT[0xa020] = psxMemRLUT[0xa040] = psxMemRLUT[0xa060] = (uint8_t *)mem_bak;
#endif
		...
		case 0x00: case 0x1e988:
			...
			for (int i = 0; i < 0x80; i++)
				psxMemWLUT[i + 0x0000] = (uint8_t*)&psxM[(i & 0x1f) << 16];
			memcpy(psxMemWLUT + 0x8000, psxMemWLUT, 0x80 * sizeof(void *));
			memcpy(psxMemWLUT + 0xa000, psxMemWLUT, 0x80 * sizeof(void *));
			...
			psxMemRLUT[0x0000] = psxMemRLUT[0x0020] = psxMemRLUT[0x0040] = psxMemRLUT[0x0060] = (uint8_t *)psxM;
			psxMemRLUT[0x8000] = psxMemRLUT[0x8020] = psxMemRLUT[0x8040] = psxMemRLUT[0x8060] = (uint8_t *)psxM;
			psxMemRLUT[0xa000] = psxMemRLUT[0xa020] = psxMemRLUT[0xa040] = psxMemRLUT[0xa060] = (uint8_t *)psxM;
#endif
...
///////////////////////////////////////////////////////////////////////////////
uint8_t psxMemRead8_direct(uint32_t mem,void *_regs) {
	const psxRegisters *regs=(psxRegisters *)_regs;
	uint32_t m = mem & 0xffff;
	switch(mem>>16) {
		case 0x1f80:
			if (m<0x1000)
				return (*(uint8_t*) &regs->psxH[m]);
			return psxHwRead8(mem);
		case 0x1f00:
			return (*(uint8_t*) &regs->psxP[m]);
		default:
			m|=mem&0x70000;
			return (*(uint8_t*) &regs->psxR[m]);
	}
}

uint16_t psxMemRead16_direct(uint32_t mem,void *_regs) {
	const psxRegisters *regs=(psxRegisters *)_regs;
	uint32_t m = mem & 0xffff;
	switch(mem>>16) {
		case 0x1f80:
			if (m<0x1000)
				return (*(uint16_t*) &regs->psxH[m]);
			return psxHwRead16(mem);
		case 0x1f00:
			return (*(uint16_t*) &regs->psxP[m]);
		default:
			m|=mem&0x70000;
			return (*(uint16_t*) &regs->psxR[m]);
	}
}

uint32_t psxMemRead32_direct(uint32_t mem,void *_regs) {
	const psxRegisters *regs=(psxRegisters *)_regs;
	uint32_t m = mem & 0xffff;
	switch(mem>>16) {
		case 0x1f80:
			if (m<0x1000)
				return (*(uint32_t*) &regs->psxH[m]);
			return psxHwRead32(mem);
		case 0x1f00:
			return (*(uint32_t*) &regs->psxP[m]);
		default:
			m|=mem&0x70000;
			return (*(uint32_t*) &regs->psxR[m]);
	}
}

void psxMemWrite8_direct(uint32_t mem, uint8_t value,void *_regs) {
	const psxRegisters *regs=(psxRegisters *)_regs;
	uint32_t m = mem & 0xffff;
	switch(mem>>16) {
		case 0x1f80:
			if (m<0x1000) *((uint8_t *)&regs->psxH[m]) = value;
			else psxHwWrite8(mem,value);
			break;
		default:
			*((uint8_t *)&regs->psxP[m]) = value;
			break;
	}
}

void psxMemWrite16_direct(uint32_t mem, uint16_t value,void *_regs) {
	const psxRegisters *regs=(psxRegisters *)_regs;
	uint32_t m = mem & 0xffff;
	switch(mem>>16) {
		case 0x1f80:
			if (m<0x1000) *((uint16_t *)&regs->psxH[m]) = value;
			else psxHwWrite16(mem,value);
			break;
		default:
			*((uint16_t *)&regs->psxP[m]) = value;
	}
}

void psxMemWrite32_direct(uint32_t mem, uint32_t value,void *_regs) {
	const psxRegisters *regs=(psxRegisters *)_regs;
	uint32_t m = mem & 0xffff;
	switch(mem>>16) {
		case 0x1f80:
			if (m<0x1000) *((uint32_t *)&regs->psxH[m]) = value;
			else psxHwWrite32(mem,value);
			break;
		default:
			*((uint32_t *)&regs->psxP[m]) = value;
	}
}
...
static inline void memstats_add_read(uint32_t addr, MemstatWidth width)
{
...
static inline void memstats_add_write(uint32_t addr, MemstatWidth width)
{
	MemstatRegion region;
	addr &= 0xfffffff;
	switch (addr >> 16) {
