...
struct dir_item {
	char	*name;
	int32_t	type; // 0=dir, 1=file, 2=zip archive
};

int compare_names(struct dir_item *a, struct dir_item *b)
{
	uint_fast8_t aIsParent = strcmp(a->name, "..") == 0;
	uint_fast8_t bIsParent = strcmp(b->name, "..") == 0;
...
static int32_t check_ext(const char *name)
{
...
static int32_t get_entry_type(char *cwd, char *d_name)
{
	int32_t type;
	struct stat item;
...
char *FileReq(char *dir, const char *ext, char *result)
{
	...
	static int32_t cursor_pos = 1;
	static int32_t first_visible;
	...
	for (;;) {
		...
		if (num_items == 0) {
			...
			while ((direntry = readdir(dirstream))) {
				int32_t type = get_entry_type(cwd, direntry->d_name);
...
static int gui_select_multicd(uint_fast8_t swapping_cd)
{
...
static int gui_swap_cd(void)
{
	...
	uint_fast8_t using_multicd = cdrIsoMultidiskCount > 1;
...
static int cycle_alter(uint32_t keys)
{
	if (keys & KEY_RIGHT) {
...
static int bios_alter(uint32_t keys)
{
	if (keys & KEY_RIGHT) {
...
static int RCntFix_alter(uint32_t keys)
{
	if (keys & KEY_RIGHT) {
...
static int Analog_Mode_alter(uint32_t keys)
{
	if (keys & KEY_RIGHT) {
		Config.AnalogMode++;
...
static int McdSlot1_alter(uint32_t keys)
{
...
static int McdSlot2_alter(uint32_t keys)
{
...
static int framelimit_alter(uint32_t keys)
{
	if (keys & KEY_RIGHT) {
...
#ifdef USE_GPULIB
static int frameskip_alter(uint32_t keys)
{
...
#ifdef GPU_UNAI
static int ntsc_fix_alter(uint32_t keys)
{
	if (keys & KEY_RIGHT) {
...
static int interlace_alter(uint32_t keys)
{
	if (keys & KEY_RIGHT) {
...
static int dithering_alter(uint32_t keys)
{
	if (keys & KEY_RIGHT) {
...
static int lighting_alter(uint32_t keys)
{
	if (keys & KEY_RIGHT) {
...
static int fast_lighting_alter(uint32_t keys)
{
	if (keys & KEY_RIGHT) {
...
static int blending_alter(uint32_t keys)
{
	if (keys & KEY_RIGHT) {
...
/*
static int pixel_skip_alter(uint32_t keys)
{
	if (keys & KEY_RIGHT) {
...
static int cdda_alter(uint32_t keys)
{
	if (keys & KEY_RIGHT) {
...
static int forcedxa_alter(uint32_t keys)
{
	if (keys & KEY_RIGHT) {
...
static int syncaudio_alter(uint32_t keys)
{
	if (keys & KEY_RIGHT) {
...
static int spuupdatefreq_alter(uint32_t keys)
{
	if (keys & KEY_RIGHT) {
...
static int spuirq_alter(uint32_t keys)
{
	if (keys & KEY_RIGHT) {
...
#ifdef SPU_PCSXREARMED
static int interpolation_alter(uint32_t keys)
{
	if (keys & KEY_RIGHT) {
...
static int reverb_alter(uint32_t keys)
{
	if (keys & KEY_RIGHT) {
...
static int volume_alter(uint32_t keys)
{
...
static int cheat_alter(uint32_t keys) {
	cheat_toggle(gui_CheatMenu.cur);
	return 0;
}
