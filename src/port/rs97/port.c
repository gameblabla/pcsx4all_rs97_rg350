...
void port_printf(int x, int y, const char *text)
{
	...
	for (int i = 0; i < len; i++) {
		...
		for (int l = 0; l < 8; l++) {
			...
			screen[pos+0] = ~screen[pos+0];
			screen[pos+1] = ~screen[pos+1];
			iscreen[pos+2] = ~screen[pos+2];
			screen[pos+3] = ~screen[pos+3];
			screen[pos+4] = ~screen[pos+4];
			screen[pos+5] = ~screen[pos+5];
