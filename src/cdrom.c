...
static unsigned int msf2sec(const uint8_t *msf) {
	return ((msf[0] * 60 + msf[1]) * 75) + msf[2];
}

// for that weird psemu API..
static unsigned int fsm2sec(const uint8_t *msf) {
	...
	switch (Irq) {
		...
		case CdlPlay:
			...
			else
			{
				...
				if (CDR_getTD((uint8_t)cdr.CurTrack, cdr.ResultTD) != -1) {
		...
		case CdlStop:
			if (cdr.Play) {
				...
				CDR_getTD((uint8_t)(cdr.CurTrack), cdr.ResultTD);
...
void cdrReadInterrupt() {
	...
	if ((!cdr.Muted) && (cdr.Mode & MODE_STRSND) && (!Config.Xa) && (cdr.FirstSector != -1)) { // CD-XA
		...
		uint_fast8_t played_ADPCM = false;   // See comments further below
