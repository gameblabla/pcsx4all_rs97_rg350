...
void cheat_load(void)
{
	...
	if (f) {
		...
		filebuf = malloc(finfo.uncompressed_size + 1);
	...
	for(;;) {
		...
		if (*buf == '#') {
			...
			if (strlen(buf) >= 13 && (buf[8] == ' ' || buf[8] == '\t')) {
				...
				code1 = (uint32_t) strtoul(buf, NULL, 16);
				buf += 9;
				while (*buf == ' ' || *buf == '\t') ++buf;
				if ((*buf < '0' || *buf > '9') && (*buf < 'A' || *buf > 'F') && (*buf < 'a' || *buf > 'f')) {
					continue;
				}
				code2 = (uint16_t) strtoul(buf, NULL, 16);
...
{
	uint32_t code1 = lines->code1;
	uint16_t code2 = lines->code2;
	switch (code1 >> 24) {
	case 0x10: {
		uint32_t addr = code1 & 0x00FFFFFFu;
	...
	case 0x80:psxMemWrite16(code1 & 0x00FFFFFFu, code2);
		return 1;
	case 0x50: {
		uint32_t cnt, addr, value, addrstep, addrend, bits;
		if (total < 2) return -1;
		cnt = (code1 >> 8) & 0xFFFFu;
